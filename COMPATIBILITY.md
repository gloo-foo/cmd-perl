...
yup-perl/cmd-perl is a thin wrapper that forks the system `perl` binary: it maps its own `-n`/`-p`/`-a` switches onto perl's, appends `-e SCRIPT`, and streams stdin through. Because the real interpreter runs the script, output is byte-identical to invoking `perl` directly with the equivalent switches (verified in the `yupsh/bin-perl` Docker integration harness against a Debian `perl` install). It exposes a deliberate subset of perl's command line — the per-line filter switches — not the full perl CLI.
...
- yup-perl exposes a subset of perl's command line: the SCRIPT (perl `-e`) plus the three per-line filter switches `-n` (`--loop`), `-p` (`--print`), and `-a` (`--autosplit`). The script is supplied as a positional operand rather than via an explicit `-e`. The many other perl options (`-i`, `-l`, `-0`, `-F`, `-M`, multiple `-e`, `@ARGV` file operands, `-w`/`-W`, etc.) are not exposed by the wrapper.
